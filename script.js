...
let x = innerWidth / 3 , y = innerHeight / 3 ;
...
class Effect{
      ...
      update(){
         ...
         this.x = x + Math.cos(this.radians) * 0;
         this.y = y + Math.sin(this.radians) * 0;
         this.circleTime--;
         this.draw();
      }
...
function onClickHandler(){
  ...
  const particleCount = 50  , particleCount1 = 50 , color = getRandomColor();
...
function animationKeyFrame(){
  ...
  circle.forEach( (loader,i) =>{
        loader.circleTime < -1 ? circle.slice(i,1) : loader.update();
  })
